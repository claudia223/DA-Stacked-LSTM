...
def init_hidden(x, hidden_size: int):
    ...
    """
    return Variable(torch.zeros(2, x.size(0), hidden_size))
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, T: int):
        ...
        self.lstm_layer = nn.LSTM(input_size=input_size, hidden_size=hidden_size, num_layers=2)
    ...
    def forward(self, input_data):
        ...
        for t in range(self.T - 1):
            ...
            x = torch.cat((hidden[-1].repeat(self.input_size, 1, 1).permute(1, 0, 2),
                           cell[-1].repeat(self.input_size, 1, 1).permute(1, 0, 2),
            ...
            hidden = hidden.view(2,-1 ,self.hidden_size) #reshaping the data for Dense layer next
            cell = cell.view(2,-1, self.hidden_size)
...
class Decoder(nn.Module):
    ...
    def __init__(self, encoder_hidden_size: int, decoder_hidden_size: int, T: int, out_feats=1):
        ...
        self.lstm_layer = nn.LSTM(input_size=out_feats, hidden_size=decoder_hidden_size,num_layers=2)
    ...
    def forward(self, input_encoded, y_history):
        ...
        for t in range(self.T - 1):
            ...
            x = torch.cat((hidden[-1].repeat(self.T - 1, 1, 1).permute(1, 0, 2),
                           cell[-1].repeat(self.T - 1, 1, 1).permute(1, 0, 2),
                           input_encoded), dim=2)
